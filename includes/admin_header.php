...
<title><?= $page_title ?> — EduTrack Admin</title>
...
.user-name { font-size: .85rem; font-weight: 600; truncate: ellipsis; }
...
.topbar-btn.primary:hover { box-shadow: 0 4px 20px rgba(79,142,247,.5); transform: translateY(-1px); }
...
.stat-card:hover { transform: translateY(-3px); box-shadow: 0 12px 32px rgba(0,0,0,.4); }
...
.stat-trend.up { background: rgba(16,185,129,.12); color: var(--green); }
.stat-trend.down { background: rgba(239,68,68,.12); color: var(--red); }
...
.btn-primary:hover { box-shadow: 0 4px 20px rgba(79,142,247,.5); transform: translateY(-1px); }
.btn-outline {
...
.modal {
  ...
  border-radius: 20px; width: 90%; max-width: 580px;
...
<div class="main">
  <div class="topbar">
    <div class="topbar-left">
      <h1><?= $page_title ?></h1>
      <div class="breadcrumb">EduTrack / <span><?= $page_title ?></span></div>
    </div>
    <div class="topbar-right">
      <?php $msg = flash('success'); if ($msg): ?>
        <span style="font-size:.85rem;color:var(--green);">✓ <?= htmlspecialchars($msg) ?></span>
      <?php endif; ?>
    </div>
  </div>
