...
$students_list = [];
if ($sel_subject) {
    $subj = $db->query("SELECT class FROM subjects WHERE id=$sel_subject")->fetch_assoc();
    if ($subj) $sel_class = $subj['class'];
}

if ($sel_class) {
    $res = $db->query("SELECT * FROM students WHERE class='$sel_class' AND status='Active' ORDER BY name");
      ...
      <div class="form-field" style="margin:0;flex:1;min-width:180px;">
        ...
        <select name="subject_id" onchange="this.form.submit()">
          ...
          <option value="<?= $s['id'] ?>" <?= $sel_subject==$s['id']?'selected':'' ?>>
            <?= htmlspecialchars($s['class'] . ' — ' . $s['name']) ?>
          </option>
          <?php endwhile; ?>
        </select>
      </div>
